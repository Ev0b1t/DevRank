...
export default function CandidatesPage() {
  ...
  return (
    <main
      ...
      style={{
        background:
          "radial-gradient(ellipse 80% 50% at 50% -10%, rgba(59,130,246,0.18) 0%, transparent 60%), #060d1a",
      }}
    >
      ...
      <div className="relative w-full max-w-6xl">
        ...
        <div className="flex items-center gap-2 mb-5">
          <span
            className="text-[11px] font-bold tracking-[0.2em] uppercase mr-1"
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                {["Candidate", "Final Score", "Trust Score", "GitHub", ""].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody>
              {loading ? (
                ...
                sortedCandidates.map((c, i) => {
                  ...
                  const sc =
                    finalScore !== null ? getScoreColors(finalScore) : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 align-middle">
                        {finalScore !== null && sc ? (
                          <div className="flex items-center gap-2.5">
                            <span
                              className={`text-[2rem] font-black leading-none tracking-tight ${sc.text}`}
                            >
                              {finalScore}
                            </span>
                            <span
                              className={`text-[10px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full border ${sc.bg} ${sc.text} ${sc.border}`}
                            >
                              {sc.label}
                            </span>
                          </div>
                        ...
                        {trustScore !== null ? (
                          <div className="flex items-center gap-2">
                            <ShieldCheck
                              size={14}
                              className={sc?.text ?? ""}
                              style={!sc ? { color: "#94a3b8" } : undefined}
                            />
                            <span
                      ...
                      <td className="px-6 py-5 align-middle text-right">
                        <Link
                          to={`/candidates/${c.id}`}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold no-underline transition-all duration-200 border"
                          style={{
